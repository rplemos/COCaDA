...
from argparse import ArgumentParser, ArgumentError, ArgumentTypeError, SUPPRESS
...
def cl_parse():
    ...
    try:
        ...
        parser.add_argument('-m', '--cores', required=False, nargs='?', const=0, help='Use MultiCore mode. Default uses all available cores, and selections can be defined based on the following: '
                            '-m X = specific single core. -m X-Y = range of cores from X to Y. -m X,Y,Z... = specific multiple cores. -m 0 = uses all available cores.')
        ...
        parser.add_argument('-r', '--region', required=False, nargs='?', help='Define only a region of residues to be analyzed. Selections can be defined based on the following: '
                            '-r X-Y = range of residues from X to Y. -r X,Y,Z... = specific multiple residues.')
        parser.add_argument('-i', '--interface', required=False, nargs='?', const='interface.csv', help=SUPPRESS)
        parser.add_argument('-d', '--distances', nargs='+', required=False,  default='contact_distances.json', 
                            help='Processes custom contact distances from a .json file. Default: "contact_distances.json". Users can also input 14 comma-separated float values ' \
        ...
        if distances != 'contact_distances.json':
            distances, modified_distances = validate_distances(distances)
        else:
...
def validate_ph(value):
    ...
    if ph == -1:
        return ph
    if not (0.1 <= ph <= 14.0):
        raise ArgumentTypeError(f"Invalid pH: Must be between 0.1 and 14 (got {ph}).")
    return ph
...
def validate_distances(value):
    ...
    if value[0].endswith(".json"):  # User passed a JSON file
        file = value[0]
        try:
        ...
        raise ArgumentTypeError("Invalid distance input! Provide either a JSON file or 14 comma-separated float values.")
    ...
    return validated_distances, modified_values
